...
const ImageTiles = (props: { images: any; rows: number; cols: number, toggled: boolean }) => {
  ...
  const Tiles = () => {
    ...
    for (let i = 0; i < size; i++) {
        ...
        imageElement.push(<img className={`${toggleValue[togglerVal]}`} key={Math.random()} src={imageUri} />);
      }
